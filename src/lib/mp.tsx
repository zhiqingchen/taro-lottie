...
class LottieView extends Component<AnimatedLottieViewProps, LottieViewState> {
  ...
  componentDidUpdate(prevProps): void {
    ...
    if (prevProps.progress !== progress) {
      this.animation.goToAndPlay(progress * this.animation.getDuration(true), true)
    }
  ...
  init() {
    return new Promise((resolve, reject)=>{
      if(this.inited) {
        ...
        const { source, loop, autoPlay, style: { width, height }, onAnimationFinish } = this.props
        createSelectorQuery().select(`#${this.id}`).node(res => {
          try {
            ...
            onAnimationFinish && this.animation.addEventListener('complete', onAnimationFinish)
            this.setSpeed()
            this.inited = true
            resolve(this.inited)
          } catch (error) {
